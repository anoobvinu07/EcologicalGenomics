...
Questions can we ask/address with this experimental design, with these data? How do we translate these questions to models in DESeq2?  
...
The index file called `Pabies_cds_index`, which contains all of the coding sequences for P. abies, not just the high confidence coding sequences. This improved the percent mapping from ~ 4% to ~ 25%. We further hypothesized that many of our reads were not mapping because the reference we had selected included only the coding region. In working with 3’ RNAseq data, much of our sequencing effort is likely to be in the 3’ UTR (untranslated region). We concatenated the reference sequences for the coding (“cds”) and the 3’ UTR (“2kb downstream”) for each gene. We then mapped to this new reference using salmon (as you had done before). Our mapping rate improved dramatically, ranging from 40-70% of reads mapping across samples, mean of 52%.   

### Ways to explore data analysis  
  1. Explore the various results for a given experimental design, see: `resultsNames(dds)`  
  
  2. Set up your DESeq object with different experimental designs; e.g., `design = ~ climate + treatment + climate:treatment`  
  
  3. Subset your data to exclude/include different factors, run different designs; `select` and `subset` are handy functions in R for subsetting your `countsMatrix` or conds table.  
